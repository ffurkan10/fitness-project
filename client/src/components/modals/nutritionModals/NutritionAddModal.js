...
import { createNutrition } from '../../../features/nutrition/NutritionSlice';

const NutritionAddModal = () => {
    ...
    const addMeals = (index) => {
        setMealsByTime((prev) => {
            const updated = [...prev];
            updated[index].meals.push({
                id: updated[index].meals.length + 1,
                name: '',
                kcal: '',
                scale: ''
            });
    ...
    const removeMeal = (timeIndex, mealIndex) => {
        setMealsByTime((prev) => {
            const updated = [...prev];
            updated[timeIndex].meals = updated[timeIndex].meals.filter((_, i) => i !== mealIndex);
            return updated;
        });
    };
    ...
    const handleSubmit = () => {
        ...
        dispatch(createNutrition(submitData))
    };
...
export default NutritionAddModal;
