...
import { getUserMembership, updateMembership } from '../../../features/membership/MembershipSlice';
...
const MembershipUpdateModal = () => {
    ...
    const {userMembership} = useSelector((state) => state.membership);
    ...
    useEffect(() => {
        dispatch(getUserMembership(selectedUser?._id))
    }, [])

    useEffect(() => {
        if (userMembership) {
            setData({
                remainingCourses: userMembership.remainingCourse,
                price: userMembership.price,
                startDate: moment(userMembership.startDate).format("YYYY-MM-DD"),
                endDate: moment(userMembership.endDate).format("YYYY-MM-DD"),
            });
    ...
    return (
        <div onClick={() => dispatch(showModal(null))} className="fixed top-0 right-0 bottom-0 left-0 bg-modalBg flex justify-center items-center z-[9999] shadow-lg">
            <div onClick={(e) => e.stopPropagation()} className="w-[600px] h-[600px] p-5 rounded-lg flex flex-col justify-between items-center bg-white gap-5 overflow-y-auto">
                ...
                <h2 className='text-dark text-xl font-bold mb-5'>Üyelik Bilgilerini Güncelle</h2>
                ...
                <div className='flex gap-4 w-full items-center justify-center'>
                    <Button handleClick={handleSubmit} text={"Güncelle"} />
