...
import { createBodyFeatures, getUserBody } from '../../../features/body/BodyFeatureSlice';

const BodyFeatureUpdateModal = () => {
   ...
   const dispatch = useDispatch();
    ...
    const { userBody } = useSelector((state) => state.bodyFeature);
    ...
    useEffect(() => {
      if (userBody) {
        setData({
            weight: userBody.weight || 0,
            height: userBody.height || 0,
            chest: userBody.chest || 0,
            waist: userBody.waist || 0,
            hips: userBody.hips || 0,
            shoulders: userBody.shoulders || 0,
            arms: userBody.arms || 0,
            legs: userBody.legs || 0,
            fatPercentage: userBody.fatPercentage || 0,
        });
      }
    }, [userBody])
  ...
  return (
    <div onClick={() => dispatch(showModal(null))} className="fixed top-0 right-0 bottom-0 left-0 bg-modalBg flex justify-center items-center z-[9999] shadow-lg">
        <div onClick={(e) => e.stopPropagation()} className="w-[600px] h-[600px] p-5 rounded-lg flex flex-col justify-between items-center bg-white gap-5 overflow-y-auto">
            ...
            <h2 className='text-dark text-xl font-bold mb-5'>Vücut Ölçülerini Güncelle</h2>
            ...
            <div className='flex gap-4 w-full items-center justify-center'>
                <Button width={"150px"} handleClick={handleSubmit} text={"Güncelle"} />
...
export default BodyFeatureUpdateModal